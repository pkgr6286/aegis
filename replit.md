...
- **Documentation System**: Highly visual React-based documentation with beautiful UI components (accessible to both Super Admin and Pharma Admin via `/docs`). Includes comprehensive User Guide with integration tutorials and complete Technical Documentation covering all 13 database tables (public, core, programs, consumer, partners schemas) and all API endpoint categories (Authentication, Account Management, Super Admin, Pharma Admin, Brand Configs, Drug Programs, Screener Versions, Regulatory, Analytics, AI Analyst, Clinician Review, Consumer Screening, EHR Integration, Partner Verification).
